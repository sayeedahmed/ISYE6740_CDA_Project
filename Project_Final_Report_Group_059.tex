...
\begin{tabular}{lrrrr}
\toprule
Room type & Listings & Median price & Mean price & Median reviews in last 365 days \\
\midrule
...
A density-based anomaly score was then created using a Gaussian mixture model in PCA space. This is related to the GMM and EM material from lecture. I did not use KDE in the final implementation. GMM was simpler to run on the full dataset and gave a direct log-density score for each listing. A listing with low estimated density received a high anomaly percentile. The final cluster anomaly score averaged the K-means distance percentile and the GMM anomaly percentile:
\[
S^{\text{cluster}}_i
...
\toprule
Candidate label & Listings & Median actual & Median predicted & Median \% above expected & Median availability \\
\midrule
...
\begin{tabular}{rrrrr}
\toprule
Final score decile & Median actual price & Median predicted price & Median \% above expected & Median recent reviews \\
\midrule
...
The main limitation is that there is no ground-truth overpricing label. A high score means that a listing looks expensive relative to the information in the dataset, not that the host made a pricing mistake. Some flagged listings may be special cases, such as monthly rentals, event-based pricing, blocked calendars, unusually high-end properties, or listings with temporary pricing choices that are not fully explained by the available variables.
...
Inside Airbnb. \textit{Get the Data}. \url{https://insideairbnb.com/get-the-data/}. Accessed June 2026.

\bibitem{insideairbnb_assumptions}
Inside Airbnb. \textit{Data Assumptions}. \url{https://insideairbnb.com/data-assumptions/}. Accessed June 2026.

\bibitem{course_notes}
ISYE 6740 course lecture notes on PCA, clustering, density estimation, Gaussian mixture models, and optimization.
\end{thebibliography}
