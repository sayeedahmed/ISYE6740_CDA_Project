...
\paragraph{Related work and positioning.} Airbnb price prediction has been studied before, so I do not treat the basic prediction task itself as the novel part of this project. Prior work has used listing, host, location, and review information to predict Airbnb prices with methods such as linear models, tree-based models, clustering, support-vector regression, neural networks, and sentiment features \cite{airbnb_ml_sentiment}. There is also prior work focused specifically on predicting Austin short-term rental prices using machine learning \cite{austin_airbnb_dynamic}. My project uses that general price-prediction idea as a starting point, but the main goal is different. Instead of only asking how accurately nightly price can be predicted, I use out-of-fold prediction residuals as a possible overpricing signal, compare those residuals within peer groups, add PCA/K-means and GMM density information, and then check the ranking against availability and recent-review proxies. This makes the project closer to a ranked screening tool than a standard price-prediction exercise.
...
Because the final score uses chosen weights, I also checked how sensitive the strongest labels were to small changes in those weights. I kept the same three score components but shifted emphasis toward residuals, cluster anomaly, or demand proxies. This was not another model-training step; it was just a practical check on whether the 13 strong candidates would stop looking strong if the final formula changed a little. The result was reassuring. All 13 strong candidates stayed above a score of 90 under every alternate weighting, and most stayed above 95. The demand-heavy version was the strictest, which makes sense because a few strong candidates had slightly weaker demand-proxy values than the others.

\begin{table}[H]
\centering
\caption{Weight sensitivity check for the 13 strong candidates from the final score.}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrrrr}
\toprule
Scenario & Residual wt. & Cluster wt. & Demand wt. & Median score & Min. score & Count $\geq$ 95 \\
\midrule
Original & 0.60 & 0.25 & 0.15 & 96.23 & 95.22 & 13 \\
Residual-heavy & 0.70 & 0.20 & 0.10 & 97.10 & 95.74 & 13 \\
Cluster-heavy & 0.55 & 0.35 & 0.10 & 96.91 & 94.86 & 12 \\
Demand-heavy & 0.50 & 0.25 & 0.25 & 95.29 & 93.84 & 9 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\bibitem{airbnb_ml_sentiment}
Kalehbasti, P. R., Nikolenko, L., and Rezaei, H. (2019). Airbnb price prediction using machine learning and sentiment analysis. \textit{arXiv preprint arXiv:1907.12665}. \url{https://arxiv.org/abs/1907.12665}.

\bibitem{austin_airbnb_dynamic}
Chapman, S., Mohammad, S., and Villegas, K. (2023). Predicting listing prices in dynamic short term rental markets using machine learning models. \textit{arXiv preprint arXiv:2308.06929}. \url{https://arxiv.org/abs/2308.06929}.
